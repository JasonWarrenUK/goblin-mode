...
<pull-request-create>
    <arguments>
        ...
        <axis name="base">The token pair `base &lt;branch&gt;` anywhere in $ARGUMENTS sets the PR's base branch. Default: `main`. A non-main base makes this a **stacked PR** layered on the parent branch's open PR; see ${CLAUDE_PLUGIN_ROOT}/references/stacked-prs.md.</axis>
    ...
    <steps>
        ...
        <step num="3">Fill the template at `${CLAUDE_PLUGIN_ROOT}/templates/pr-description.md` exactly; each {{ slot }} describes its content. For wordy style, omit the Screenshots block entirely (and its trailing `---`). For shiny style, one collapsible `&lt;details&gt;` per named screenshot.</step>
