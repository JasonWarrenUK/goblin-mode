...
description: "Interview-led audit of the roadmap's dependency graph: are the edges, gates and milestone boundaries actually rational?"
...
- **Gate rationality**: for every external gate, is it still genuinely external, still imposing the right status (`blocked`/`paused`/`deferred`), and is its `blocks[]` list still the honest set?
