...
<pull-request-create>
    ...
    <steps>
        ...
        <step num="6">On approval, create the PR with `gh pr create --base &lt;base&gt;`; add `--draft` when mode is draft. For a non-main base, then link it into the parent's stack (`gh stack link &lt;parent-pr&gt; &lt;new-pr&gt;`); under `auto` do this immediately, otherwise offer it. Report the PR URL.</step>
    </steps>
    <rules>
