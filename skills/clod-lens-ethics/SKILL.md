...
- Colour contrast ratios meet WCAG 2.1 AA minimum (4.5:1 for normal text, 3:1 for large text); project themes ship a `contrast` table that `validate.ts` checks, so pairs taken from the theme are verified rather than assumed
