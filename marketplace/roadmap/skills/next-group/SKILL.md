...
Run `python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/roadmap.py detect`. Exit **3** = old simple format: tell the user to run `roadmap:migrate` first and stop. Exit **2** = could not locate/parse; ask for the path. Proceed only on exit 0.
