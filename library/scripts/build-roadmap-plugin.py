...
# (situation, plugin skill dir) for the README table
README_ROWS = [
	("No roadmap yet", "create"),
	("Old single-file format detected", "migrate"),
	("Half-formed ideas to explore into tasks", "create-interview"),
	("Add a task, a chain of tasks or a milestone", "update-tasks"),
	("Tasks need owners, or a dev's load needs handing over", "update-devs"),
	("Work landed / statuses drifted (add `reconcile` to check against code)", "maintain"),
	("Priorities, freshness or dependency-graph review", "review"),
	("Render the HTML dashboard", "dashboard"),
	("Choose the next task (one pick)", "next-suggest"),
	("See the whole ready-set", "next-group"),
]
...
def build(out: Path) -> None:
	...
	rows = "\n".join(f"| {situation} | `/{PLUGIN_NAME}:{skill}` |" for situation, skill in README_ROWS)
	(out / "README.md").write_text(README.replace("{rows}", rows))
